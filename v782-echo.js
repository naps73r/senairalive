(() => {
  ...
  root.classList.add("alaina-x-senaira-silk");

  function applySilkClassToBody() {
    document.body?.classList.add("alaina-x-senaira-silk");
  }
  ...
  function updateViewport() {
    ...
    if (height !== lastHeight) {
      root.style.setProperty("--alaina-x-senaira-visible-height", `${height}px`);
      lastHeight = height;
    }

    root.style.setProperty("--alaina-x-senaira-visible-width", `${width}px`);

    root.classList.toggle("alaina-x-senaira-silk-very-short", height < 500);
    root.classList.toggle("alaina-x-senaira-silk-short", height >= 500 && height < 610);
    root.classList.toggle("alaina-x-senaira-silk-medium", height >= 610);
